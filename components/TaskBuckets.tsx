...
function StatusPill({
  ...
  if (!task.listId) {
    return task.status ? (
      <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full flex-shrink-0 bg-sand2 text-ink3">
        {task.status}
      </span>
    ) : null;
  }
...
function TaskRow({
  ...
  tasks,
  onStatusChange,
}: RowComponentProps<{ tasks: Task[]; onStatusChange: StatusChangeHandler }>) {
  const t = tasks[index];
  ...
  return (
    <div style={style} className="px-0.5">
      <div className="h-full flex items-center gap-3 border-b border-sand3 hover:bg-sand2 px-1 transition-colors group">
        {(isUrgent || isHigh) && (
          <span
            className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full flex-shrink-0 ${isUrgent ? "bg-danger-light text-danger" : "bg-warning-light text-warning"}`}
          >
        ...
        >
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium group-hover:text-accent leading-tight truncate">
              {t.name}
            </div>
            <div className="text-[11px] text-ink4 mt-0.5 truncate">
              {t.list}
              {t.dueDate ? ` · Due ${t.dueDate}` : ""}
            </div>
          </div>
          <span className="text-ink4 text-sm flex-shrink-0 group-hover:text-accent">
            ↗
          </span>
        </a>
...
function Accordion({
  ...
  return (
    <div className="border-b border-sand3 last:border-0">
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center gap-2 py-2.5 text-left hover:bg-sand2 -mx-1 px-1 rounded transition-colors"
      >
      ...
      {open && (
        <div className="pb-2">
          <List
            ...
            rowHeight={ROW}
            rowProps={{ tasks, onStatusChange }}
            style={{ height: Math.min(tasks.length * ROW, MAX_H) }}
          />
        </div>
      )}
    </div>
...
export default function TaskBuckets({
  ...
  if (tasks.length === 0)
    return (
      <p className="text-sm text-ink3 py-3">
        No active tasks found in ClickUp.
      </p>
    );
      ...
      {order.map(
        (b) =>
          groups[b].length > 0 && (
            <Accordion
              key={b}
              label={DUE_BUCKET_META[b].label}
              dot={DUE_BUCKET_META[b].dot}
              text={DUE_BUCKET_META[b].text}
              tasks={groups[b]}
              defaultOpen={b === firstOpen}
