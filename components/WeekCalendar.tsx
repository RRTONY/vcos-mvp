...
export default function WeekCalendar({
  ...
  const [viewMonth, setViewMonth] = useState(
    () =>
      new Date(
        Date.UTC(
          selectedMonday.getUTCFullYear(),
          selectedMonday.getUTCMonth(),
          1,
          12,
        ),
      ),
  ...
  const cells: (Date | null)[] = [
    ...
    ...Array.from(
      { length: daysInMonth },
      (_, i) => new Date(Date.UTC(year, month, i + 1, 12)),
    ),
  ];
  ...
  function pick(day: Date) {
    if (maxWeekTime !== null && mostRecentMonday(day).getTime() > maxWeekTime)
      return;
  ...
  return (
    <div className={`relative ${compact ? "" : "w-full"}`} ref={ref}>
      ...
      {open && (
        <div
          className={`absolute z-50 mt-1 bg-sand border border-sand4 rounded-lg shadow-card-md p-3 w-[17rem] ${compact ? "right-0" : "left-0"}`}
        >
          {/* Month nav */}
          <div className="flex items-center justify-between mb-2">
            <button
              type="button"
              onClick={() =>
                setViewMonth(new Date(Date.UTC(year, month - 1, 1, 12)))
              }
          ...
          <div className="grid grid-cols-7 gap-0.5 mb-1">
            {WEEKDAYS.map((d, i) => (
              <div
                key={i}
                className="text-center text-[10px] font-bold text-ink4"
              >
                {d}
              </div>
            ))}
          ...
          <div className="grid grid-cols-7 gap-0.5">
            {cells.map((day, i) => {
              ...
              const isCurrentWeek =
                mostRecentMonday(day).getTime() === currentWeekTime;
              const isToday = day.toDateString() === new Date().toDateString();
              const disabled =
                maxWeekTime !== null &&
                mostRecentMonday(day).getTime() > maxWeekTime;
              return (
                <button
                  key={i}
                  type="button"
                  ...
                  title={
                    disabled
                      ? "Future weeks can't be selected"
                      : fmtWeek(mostRecentMonday(day))
                  }
                >
                  {day.getDate()}
                </button>
