...
function parseBlocks(md: string): Block[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isTableRow(line) && i + 1 < lines.length && isSeparator(lines[i + 1])) {
      ...
      blocks.push({ type: 'table', header, rows }); i = j - 1; n = 0; continue
    }
    const h = line.match(/^(#{1,4})\s+(.*)$/)
    if (h) { blocks.push({ type: 'h', level: h[1].length, text: h[2] }); n = 0; continue }
    const ol = line.match(/^\s*(\d+)[.)]\s+(.*)$/)
    if (ol) { n++; blocks.push({ type: 'li', ordered: true, index: n, text: ol[2] }); continue }
    const ul = line.match(/^\s*[-*•]\s+(.*)$/)
    if (ul) { blocks.push({ type: 'li', ordered: false, index: 0, text: ul[1] }); n = 0; continue }
    blocks.push({ type: 'p', text: line }); n = 0
  }
  return blocks
}
...
function safeName(title: string, ext: string): string {
  const base = (title || 'vcos-report').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 60) || 'vcos-report'
  return `${base}.${ext}`
}
...
function bodyToHtml(text: string): string {
  const parts: string[] = []
  let ul: string[] = []; let ol: string[] = []
  const flushUl = () => { if (ul.length) { parts.push(`<ul>${ul.map(t => `<li>${inlineToHtml(t)}</li>`).join('')}</ul>`); ul = [] } }
  const flushOl = () => { if (ol.length) { parts.push(`<ol>${ol.map(t => `<li>${inlineToHtml(t)}</li>`).join('')}</ol>`); ol = [] } }
  const flush = () => { flushUl(); flushOl() }
  for (const b of parseBlocks(text)) {
    if (b.type === 'li') { if (b.ordered) { flushUl(); ol.push(b.text) } else { flushOl(); ul.push(b.text) } continue }
    flush()
    if (b.type === 'h') { const lv = Math.min(b.level, 4); parts.push(`<h${lv}>${inlineToHtml(b.text)}</h${lv}>`); continue }
    if (b.type === 'table') {
      const thead = `<tr>${b.header.map(c => `<th>${inlineToHtml(c)}</th>`).join('')}</tr>`
      const tbody = b.rows.map(r => `<tr>${b.header.map((_, ci) => `<td>${inlineToHtml(r[ci] ?? '')}</td>`).join('')}</tr>`).join('')
...
function wrapDocument(title: string, body: string, word: boolean): string {
  const ns = word ? ' xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"' : ''
...
export function exportToPdf(text: string, title = 'VCoS-AI Report') {
  const html = wrapDocument(safeName(title, 'pdf').replace(/\.pdf$/, ''), bodyToHtml(text), false)
...
export function exportToDoc(text: string, title = 'VCoS-AI Report') {
  ...
  a.href = url; a.download = safeName(title, 'doc')
  document.body.appendChild(a); a.click(); a.remove()
  setTimeout(() => URL.revokeObjectURL(url), 1000)
}

export function exportReport(text: string, format: ExportFormat, title?: string) {
