...
export async function buildSlackMessagesSnapshot(): Promise<{
  messages: SlackMessage[];
  byChannel: Record<string, number>;
  channelCount: number;
}> {
  const oldest = String(
    Math.floor((Date.now() - INGEST_DAYS * 86400000) / 1000),
  );
  ...
  for (const u of users)
    if (u.id) userMap[u.id] = u.real_name ?? u.name ?? "Unknown";

  const active = channels
    .filter((c) => c.is_member && !c.is_archived)
    .slice(0, MAX_INGEST_CHANNELS);
...
function getMostRecentMonday(from: Date): Date {
  return getMondayOfWeekPT(from);
}

export function weekLabel(): string {
  return fmtWeekRange(getMondayOfWeekPT());
}
...
function getPostersFromMessages(
  ...
  const window = messages.filter((m) => {
    ...
    return (
      ts >= fromTs &&
      ts < toTs &&
      (m.text ?? "").toLowerCase().includes("#myweeklyreport")
    );
  });
  return {
    names: Array.from(
      new Set([
        ...
        ...window
          .filter((m) => m.subtype === "bot_message" && m.username)
          .map((m) => m.username!),
      ]),
    ),
    handles: Array.from(
      new Set(
        window.map((m) => (m.user ? handleMap[m.user] : "")).filter(Boolean),
      ),
    ),
  };
}
...
export async function buildSlackSnapshot() {
  ...
  const [historyData, membersData, channelsData, teamMembers] =
    await Promise.all([
  ...
  const messages: Array<{
    ...
    ts?: string;
    text?: string;
  }> = (historyData as { messages?: typeof messages }).messages ?? [];

  // Week 1 and Week 2 filing detection
  const week1 = getPostersFromMessages(
    messages,
    userMap,
    handleMap,
    week1Start.getTime(),
  ...
  const week2 = getPostersFromMessages(
    messages,
    userMap,
    handleMap,
    week2Start.getTime(),
  ...
  const missing = allMemberNames.filter(
    (n) => reportMembers.some((m) => m.full_name === n) && !filed.includes(n),
  );
  ...
  const fri1 = new Date(week1Start);
  fri1.setDate(week1Start.getDate() + 4);
  const fri2 = new Date(week2Start);
  fri2.setDate(week2Start.getDate() + 4);
  ...
  return {
    ...
    slackStats: {
      ...
      channels: ((channelsData as { channels?: unknown[] }).channels ?? [])
        .length,
      messagesByDay,
    },
  };
