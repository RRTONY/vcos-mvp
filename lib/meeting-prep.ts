...
import { FiCheckCircle, FiTarget, FiAlertTriangle, FiUsers, FiAlertCircle } from 'react-icons/fi'
import { ptDateISO } from './week-utils'

export type MeetingType = 'monday' | 'thursday'
export type MeetingFieldKey = 'wins' | 'priorities' | 'blockers' | 'decisions' | 'fyis'
...
export const MEETING_PREP_CATEGORIES: { key: MeetingFieldKey; Icon: IconType; title: string; prompt: string; optional?: boolean }[] = [
  { key: 'wins', Icon: FiCheckCircle, title: 'Wins', prompt: 'What did you complete or accomplish since the last meeting?' },
  { key: 'priorities', Icon: FiTarget, title: 'Top Priorities', prompt: 'What are your top priorities until the next meeting?' },
  { key: 'blockers', Icon: FiAlertTriangle, title: 'Blockers / Help Needed', prompt: 'Anything preventing progress? Who or what are you waiting on?' },
  { key: 'decisions', Icon: FiUsers, title: 'Decisions Needed', prompt: 'Anything requiring leadership discussion or approval?' },
  { key: 'fyis', Icon: FiAlertCircle, title: 'FYIs / Risks', prompt: 'Anything important the leadership team should know?', optional: true },
]
...
function ptDayOfWeek(d: Date): number {
  const name = new Intl.DateTimeFormat('en-US', { timeZone: REPORT_TZ, weekday: 'short' }).format(d)
  return WEEKDAY_SHORT.indexOf(name)
}

function ptDateParts(d: Date): { y: number; mo: number; day: number } {
  const parts = new Intl.DateTimeFormat('en-US', {
    timeZone: REPORT_TZ, year: 'numeric', month: 'numeric', day: 'numeric',
  }).formatToParts(d)
...
export function adjacentMeetingDate(meetingDate: Date, dir: 1 | -1): Date {
  ...
  const delta = type === 'monday'
    ? (dir === 1 ? 3 : -4)   // Mon → next Thu (+3) or prior Thu (-4)
    : (dir === 1 ? 4 : -3)   // Thu → next Mon (+4) or same-week Mon (-3)
...
export function meetingDeadlineDate(meetingDate: Date): Date {
  return addDays(meetingDate, meetingTypeOf(meetingDate) === 'monday' ? -3 : -1)
}

/** True once the deadline day has fully elapsed in PT. */
export function meetingDeadlinePassed(meetingDate: Date, now: Date = new Date()): boolean {
...
export function fmtMeetingDate(date: Date): string {
  return new Intl.DateTimeFormat('en-US', { timeZone: REPORT_TZ, weekday: 'long', month: 'short', day: 'numeric' }).format(date)
}

/** "Fri, Jul 3" style label for the submission deadline. */
export function fmtDeadline(meetingDate: Date): string {
  return new Intl.DateTimeFormat('en-US', { timeZone: REPORT_TZ, weekday: 'short', month: 'short', day: 'numeric' }).format(meetingDeadlineDate(meetingDate))
}
