...
export function fmtWeekRange(monday: Date): string {
  const friday = new Date(Date.UTC(
    monday.getUTCFullYear(), monday.getUTCMonth(), monday.getUTCDate() + 4, 12,
  ))
  const fmt = (d: Date) => d.toLocaleDateString('en-US', { timeZone: REPORT_TZ, month: 'short', day: 'numeric' })
...
export function weekLabelVariants(monday: Date): [string, string] {
  ...
  const prevDay = new Date(Date.UTC(monday.getUTCFullYear(), monday.getUTCMonth(), monday.getUTCDate() - 1, 12))
