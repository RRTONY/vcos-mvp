      ...
      {showForm && (
        <div className="space-y-1.5 bg-sand2 p-2 border border-sand3">
          <div className="flex gap-1 flex-wrap">
            {(["Call", "Email", "Meeting", "Note"] as Activity["type"][]).map(
              (t) => (
                <button
                  key={t}
                  onClick={() => setType(t)}
          ...
          <div className="flex gap-1">
            ...
            <button
              ...
              className="text-[10px] text-ink4 hover:text-ink underline px-1"
            >
              Cancel
            </button>
          </div>
    ...
    >
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {(deal.year1_value ?? 0) > 0 && (
            <span className="text-xs font-mono text-ink3">
              {fmt(deal.year1_value!)}
            </span>
          )}
          {isAdmin && (
            <button
              onClick={() => onEdit(deal)}
              className="text-ink4 hover:text-ink text-xs px-1"
            >
              ✎
            </button>
          )}
          <button
            onClick={() => setExpanded((v) => !v)}
            className="text-ink4 text-xs px-0.5"
          >
            {expanded ? "▲" : "▼"}
          </button>
        </div>
      ...
      {deal.next_action && (
        <div className="text-[11px] text-ink3 border-t border-sand3 pt-2 leading-snug">
          → {deal.next_action}
          {deal.next_action_by && (
            <span className="text-ink4"> · {deal.next_action_by}</span>
          )}
        </div>
      )}
      ...
      {expanded && <ActivityLog dealId={deal.id} canAdd={true} />}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-sand w-full max-w-full sm:max-w-lg max-h-[90vh] overflow-y-auto border border-sand3 shadow-xl">
        <div className="flex items-center justify-between px-4 py-3 border-b border-sand3 sticky top-0 bg-sand">
          ...
          <button
            onClick={onClose}
            className="text-ink4 hover:text-ink text-lg leading-none p-2 -mr-2"
          >
            ✕
          </button>
        </div>
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              >
                ...
                {["Inbound", "Referral", "Outbound", "Conference", "Other"].map(
                  (s) => (
                    <option key={s}>{s}</option>
                  ),
                )}
          ...
          <div>
            ...
            <input
              ...
              placeholder="Send LOI, Schedule follow-up call..."
            />
          </div>
          <div>
            ...
            <select
              ...
              onChange={(e) => set("next_action_by", e.target.value)}
            >
              <option value="">- Select -</option>
              {teamNames.map((n: string) => (
                <option key={n}>{n}</option>
          ...
          <div>
            ...
            <input
              className="field-input"
              value={form.ramprate_cut}
              onChange={(e) => set("ramprate_cut", e.target.value)}
              placeholder="8% + $50K/mo"
            />
          </div>
          <div>
...
export default function BdPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setKpi({
        teamHours:
          ww?.members?.reduce(
            (s: number, m: { totalHours: number }) => s + m.totalHours,
            0,
          ) ?? 0,
  ...
  async function handleSave(data: Omit<BdDeal, "id">) {
    ...
    if (editDeal?.id) {
      ...
      setDeals((prev) =>
        prev.map((d) => (d.id === editDeal.id ? { ...d, ...data } : d)),
      );
    } else {
      // Create
    ...
    <div className="pt-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {isAdmin && (
            <button
              onClick={() => setEditDeal(EMPTY_DEAL)}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          ...
          <div
            key={t.label}
            className={`border p-3 ${t.alert ? "border-red-300 bg-red-50" : "border-sand3"} ${dealsLoading ? "animate-pulse" : ""}`}
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            ...
            },
            {
              value: kpiLoading
                ? "…"
                : `${filedMembers.length}/${reportMembers.length}`,
              label: "Reports Filed",
              sub:
            ...
            <div
              key={t.label}
              className={`border p-3 ${t.alert ? "border-red-300 bg-red-50" : "border-sand3"} ${kpiLoading ? "animate-pulse" : ""}`}
            >
            ...
            {STAGES.map((stage) => {
              ...
              return (
                <div key={stage} className="space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    <span
                      className={`text-xs font-bold px-2 py-0.5 ${STAGE_COLOR[stage]}`}
                    >
                      {stage.toUpperCase()}
                    </span>
          ...
          <div className="card divide-y divide-sand3">
            {[...(kpi?.memberHours ?? [])]
              ...
              .map((m) => (
                ...
                >
                  <span className="text-sm capitalize w-24 flex-shrink-0">
                    {m.username}
                  </span>
