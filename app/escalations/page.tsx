  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/team", { cache: "no-store" })
        .then((r) => r.json())
        .catch(() => []),
      fetch(`/api/weekly-reports?week_start=${monday}`, { cache: "no-store" })
        .then((r) => r.json())
        .catch(() => []),
      ...
      setDeals(
        Array.isArray(bd) ? bd : Array.isArray(bd?.deals) ? bd.deals : [],
      );
      setLoading(false);
    });
  }, [refreshKey]);
  ...
  const escalations: Escalation[] = useMemo(() => {
    ...
    for (const t of overdue
      ...
      out.push({
        ...
        kind: "Task",
        title: t.name,
        detail: `Urgent & overdue${t.dueDate ? ` · was due ${t.dueDate}` : ""} · ${t.list}`,
        url: t.url,
      });
    }
    ...
    for (const m of team.filter((m) => m.files_report)) {
      const rep = reports.find(
        (r) =>
          ...
          r.submitted_by
            .toLowerCase()
            .includes(m.full_name.split(" ")[0].toLowerCase()),
      );
      if (reportState(monday, rep?.created_at ?? null, now) === "missing") {
        out.push({
          sev: "high",
          kind: "Report",
          title: `${m.full_name} - weekly report not submitted`,
          detail: "Friday deadline passed",
        });
      }
    }

    // 3. Overdue commitments → high
    for (const c of commits.filter(
      ...
      out.push({
        ...
        kind: "Commitment",
        title: c.text,
        detail: `Overdue${c.owner ? ` · ${c.owner}` : ""} · was due ${c.due}`,
      });
    }
    ...
    for (const d of deals) {
      if (d.last_contact && daysSince(d.last_contact) >= DEAL_COLD_DAYS) {
        out.push({
          sev: "watch",
          kind: "Deal",
          title: `${d.company} - gone cold`,
          detail: `No contact in ${daysSince(d.last_contact)}d · ${d.stage}${d.next_action ? ` · next: ${d.next_action}` : ""}`,
        });
      }
    }
    ...
    <div className="mt-6">
      <div className="flex items-center justify-between mb-1 gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {!loading && (
            <ShareSlackButton label="Post to Slack" message={slackMsg} />
          )}
        </div>
      </div>
      <p className="text-xs text-ink4 mb-4">
        Everything that needs attention right now - overdue urgent tasks, missed
        reports, overdue commitments, and cold deals - ranked by severity.
      </p>

      {loading ? (
        <div className="py-6">
          <Spinner label="Scanning…" className="text-ink4 text-sm" />
        </div>
          ...
          {escalations.map((e, i) => {
            ...
            return e.url ? (
              <a
                ...
                className="block hover:bg-sand2 transition-colors"
              >
                {body}
              </a>
            ) : (
