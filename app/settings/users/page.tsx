...
export default function UsersPage() {
  ...
  async function resetPassword(user: User) {
    ...
    const d = await res.json();
    if (res.ok)
      setMsg(`🔑 New password for ${user.username}: ${d.tempPassword}`);
  }
  ...
  async function runBootstrap() {
    ...
    const summary = d.results
      ?.map(
        (r: { username: string; status: string }) =>
          `${r.username}: ${r.status}`,
      )
      .join(", ");
  ...
  if (loading)
    return <div className="text-ink4 text-sm pt-8">Loading users…</div>;
      ...
      <div className="card divide-y divide-border">
          ...
          <div key={user.id} className="flex items-center gap-3 px-4 py-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-xs px-1.5 py-0.5 rounded ${user.role === "owner" ? "bg-black text-white" : user.role === "admin" ? "bg-amber-100 text-amber-700" : "bg-blue-50 text-blue-600"}`}
                >
                  {user.role}
                </span>
              ...
              {user.email && (
                <div className="text-xs text-ink3 mt-0.5">{user.email}</div>
              )}
            </div>
        ...
        {users.length === 0 && (
          <div className="px-4 py-6 text-center text-ink4 text-sm">
            No users yet - click "Seed from ENV" to import existing logins, or
            "Add User" to create one.
          </div>
        )}
      </div>
    </div>
