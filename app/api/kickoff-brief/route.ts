...
export async function GET() {
  ...
  return NextResponse.json({
    brief: row?.data ?? null,
    generatedAt: row?.fetched_at ?? null,
  });
}
...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey)
    return NextResponse.json(
      { error: "ANTHROPIC_API_KEY_REPORTS not configured" },
      { status: 500 },
    );
  ...
  const [{ data: reports }, members, cu] = await Promise.all([
    sb
      .from("weekly_reports")
      .select(
        "submitted_by, created_at, blockers, escalations, priorities, goals_met, win, accomplishments, support_needed",
      )
  ...
  const missing = reportingMembers
    .filter((m) => !submittedNames.has(m.full_name))
    .map((m) => m.full_name);
  ...
  const cuByPerson = reportingMembers.map((m) => {
    ...
    const stats = cu?.assigneeStats
      ? Object.entries(cu.assigneeStats).find(([k]) => k.includes(cuKey))?.[1]
      : null;
    return {
      name: m.full_name,
      total: stats?.total ?? 0,
  ...
  const context = {
    ...
    reports: reportRows.map((r) => ({
      ...
      lastWeek: r.goals_met,
      win: r.win,
      accomplishments: r.accomplishments,
      support: r.support_needed,
    })),
