...
export async function POST(req: NextRequest) {
  ...
  if (!isCron && !isAdmin) return NextResponse.json({ error: 'Admin access required' }, { status: 403 })

  try {
    const text = await generateBrief()
    await recordSuccess(SOURCE, { text, generatedAt: new Date().toISOString() })

    let posted = false
    if (process.env.SLACK_BOT_TOKEN && text) {
      const channel = process.env.SLACK_BRIEF_CHANNEL ?? SLACK_ADMIN_CHANNEL
      const today = new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })
      await postMessage(channel, `:sunrise: *VCoS-AI — Morning Brief · ${today}*\n\n${toSlackMrkdwn(text)}`)
      posted = true
    }
    return NextResponse.json({ ok: true, posted, brief: text })
  } catch (err) {
    return NextResponse.json({ error: err instanceof Error ? err.message : 'Brief generation failed' }, { status: 502 })
  }
}
