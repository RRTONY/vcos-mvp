...
async function generateDailyReport() {
  const today = todayPT();
  const baseUrl =
    process.env.ALLOWED_ORIGIN?.replace(/\/$/, "") ?? "http://localhost:3000";

  // Fetch all sources in parallel using snapshot builders + systems status
  const [slackRes, clickupRes, webworkRes, firefliesRes, systemsRes] =
  ...
  await Promise.allSettled([
    ...
    firefliesRes.status === "fulfilled"
      ? setCache("fireflies", firefliesRes.value)
      : Promise.resolve(),
  ]);
  ...
  const totalHours = Object.values(teamHours).reduce(
    (s: number, h) => s + (h as number),
    0,
  );
  ...
  const slackMsg = [
    ...
    missing.length
      ? `❌ Missing (${missing.length}): ${missing.map((n) => n.split(" ")[0]).join(" · ")}`
      : `✅ All reports filed`,
    ``,
  ...
  if (slackToken) {
    ...
    if (missing.length > 0) {
      ...
      await fetch("https://slack.com/api/chat.postMessage", {
        ...
        body: JSON.stringify({
          channel: WEEKLY_REPORTS_CHANNEL,
          text: reminderMsg,
        }),
      });
    }
...
export async function GET(req: NextRequest) {
  ...
  const { data } = await supabase
    .from("vcos_daily_reports")
    .select(
      "report_date, reports_filed, reports_missing, overdue_count, urgent_count, total_tasks, team_hours, slack_message_ts",
    )
