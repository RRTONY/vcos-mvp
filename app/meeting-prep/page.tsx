...
const EMPTY_FIELDS: Record<MeetingFieldKey, string> = {
  wins: "",
  priorities: "",
  blockers: "",
  decisions: "",
...
export default function MeetingPrepPage() {
  ...
  const [fields, setFields] =
    useState<Record<MeetingFieldKey, string>>(EMPTY_FIELDS);
  ...
  useEffect(() => {
    if (lockedName) {
      setSelectedName(lockedName);
      return;
    }
    if (!selectedName && participantNames.length > 0)
      setSelectedName(participantNames[0]);
    ...
    fetch(`/api/meeting-prep?meeting_date=${iso}`, { cache: "no-store" })
      ...
      .then((data: MeetingPrepRow[]) => {
        const mine = Array.isArray(data)
          ? data.find((s) => s.submitted_by === activeName)
          : null;
        setFields(
          mine
            ? {
  ...
  async function handleSave() {
    if (!activeName) {
      toast("Please select a name first");
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-ink3 mb-4">
        Submit a short pre-meeting update before every Monday and Thursday
        leadership meeting - following the Mochary Method, updates are reviewed
        in advance so meeting time goes to discussion, decisions, and blockers,
        not status reads.
      </p>
      ...
      <div className="flex flex-wrap items-center gap-x-1 gap-y-2 mb-4">
        ...
        {!isCurrentMeeting && (
          <button
            onClick={() => setMeetingDate(nextMeetingDate())}
        ...
        >
          {deadlinePassed
            ? `Submissions closed (were due ${fmtDeadline(meetingDate)})`
      ...
      {locked && (
        <div className="alert alert-amber items-center">
          ...
          <span>
            The submission deadline for this meeting has passed - your update is
            now read-only.
          </span>
        </div>
      )}
          ...
          <div>
            ...
            {isAdmin ? (
              ...
              <div className="text-sm text-danger">
                Your account isn&apos;t linked to a team member, so you
                can&apos;t submit an update. Please contact an admin.
              </div>
            )}
          </div>
              ...
              {MEETING_PREP_CATEGORIES.map((cat) => (
                <div key={cat.key}>
                  <label className="text-xs font-bold uppercase tracking-widest text-ink3 flex items-center gap-1.5 mb-1">
                    ...
                    {cat.title}{" "}
                    {cat.optional && (
                      <span className="normal-case font-medium text-ink4">
                        (Optional)
                      </span>
                    )}
                  </label>
