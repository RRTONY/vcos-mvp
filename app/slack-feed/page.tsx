...
export default function SlackFeedPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    fetch("/api/slack-messages", { cache: "no-store" })
      .then((r) => r.json())
      .then((d) => {
  ...
  return (
    <div className="mt-6">
      ...
      <p className="text-xs text-ink4 mb-4">
        Recent messages from channels VCoS-AI can read - also fed into the
        assistant so it knows what&apos;s happening in Slack.
      </p>
      ...
      {!loading && channels.length > 0 && (
        <div className="flex gap-2 overflow-x-auto pb-2 mb-2">
          <button
            onClick={() => setChannel("")}
            className={`text-xs font-semibold px-3 py-1.5 rounded-full border whitespace-nowrap ${!channel ? "bg-accent text-white border-accent" : "border-sand4 text-ink3 hover:border-accent"}`}
          >
            All ({messages.length})
          </button>
          {channels.map((c) => (
            <button
              ...
              className={`text-xs font-semibold px-3 py-1.5 rounded-full border whitespace-nowrap ${channel === c ? "bg-accent text-white border-accent" : "border-sand4 text-ink3 hover:border-accent"}`}
            >
              #{c} ({byChannel[c]})
            </button>
          ))}
      ...
      {loading ? (
        <div className="py-6">
          <Spinner label="Loading Slack feed…" className="text-ink4 text-sm" />
        </div>
      ) : messages.length === 0 ? (
        <div className="card p-6 text-center text-sm text-ink4">
          No messages ingested yet. Click Refresh (the bot must be a member of
          the channels).{" "}
        </div>
      ) : (
        <div className="card divide-y divide-sand3">
          {shown.map((m, i) => (
            <div key={i} className="px-4 py-3">
              ...
              <div className="text-sm text-ink2 whitespace-pre-wrap break-words">
                {m.text}
              </div>
            </div>
          ))}
