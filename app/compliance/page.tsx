...
const BT_ITEMS: { key: keyof CheckState; label: string }[] = [
  {
    ...
    label: "Braintrust invoice submitted this period?",
  },
  {
    key: "webworkConfirmed",
    label: "WebWork screenshots cover full work period?",
  },
  {
    key: "emailMeterConfirmed",
    label: "Email Meter report submitted for this week?",
  },
  {
    key: "slackReportConfirmed",
    label: "Slack weekly report posted and linked in #weeklyreports?",
  },
];
...
export default function CompliancePage() {
        ...
        (
          ...
          setTeam(
            active.map((m) => ({
              ...
              filed: false,
              filedByWeek: [],
              btAlias: m.bills_hours
                ? (m.braintrust_name ?? m.full_name.split(" ")[0].toLowerCase())
                : null,
              btFiled: false,
            })),
          );
  ...
  async function sendReminders(names: string[]) {
    ...
    try {
      ...
      if (!res.ok) {
        toast(`Reminder failed: ${d.error ?? "error"}`);
        return;
      }
  ...
  useEffect(() => {
    ...
    Promise.all(
      mondays.map((mon) =>
        fetch(`/api/weekly-reports?week_start=${weekStartISO(mon)}`, {
          cache: "no-store",
        })
          .then((r) => r.json())
          .catch(() => []),
  ...
  const missing = deadlinePassed
    ? visibleTeam.filter((m) => m.filesReport && !m.filed)
    : [];
  const pending = !deadlinePassed
    ? visibleTeam.filter((m) => m.filesReport && !m.filed)
    : [];
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b border-sand3">
                <th className="text-left py-2 font-extrabold text-xs uppercase tracking-widest text-ink3 sticky left-0 bg-surface">
                  Team Member
                </th>
                <th className="text-left py-2 font-extrabold text-xs uppercase tracking-widest text-ink3 hidden sm:table-cell">
                  Role
                </th>
                {isOwner && (
                  <th className="text-right py-2 px-2 font-extrabold text-xs uppercase tracking-widest text-ink3 hidden sm:table-cell">
                    Rate
                  </th>
                )}
                {weekCols.map((c) => (
                  <th
                    key={c.key}
                    className="text-center py-2 px-1 font-extrabold text-[10px] font-mono text-ink3 whitespace-nowrap"
                    title={c.key}
                  >
                    {c.label}
                  </th>
                ))}
                <th className="text-center py-2 px-2 font-extrabold text-xs uppercase tracking-widest text-ink3">
                  Filed
                </th>
                <th className="text-center py-2 font-extrabold text-xs uppercase tracking-widest text-ink3 hidden sm:table-cell">
                  Braintrust
                </th>
              </tr>
            </thead>
            <tbody>
              {visibleTeam.map((m) => {
                ...
                return (
                  <tr
                    key={m.name}
                    className="border-b border-sand3 last:border-0"
                  >
                    <td className="py-2.5 font-bold sticky left-0 bg-surface">
                      {m.name}
                    </td>
                    <td className="py-2.5 text-ink3 text-xs hidden sm:table-cell">
                      {m.role}
                    </td>
                    {isOwner && (
                      <td
                        className={`py-2.5 px-2 font-mono font-bold text-right hidden sm:table-cell ${rateColor}`}
                      >
                        {m.rate}%
                      </td>
                    )}
                    {weekCols.map((c, i) => {
                      ...
                      const pendingCell =
                        isCurrentWeek && !deadlinePassed && !m.filedByWeek[i];
                      return (
                        <td key={c.key} className="py-2.5 px-1 text-center">
                          {!m.filesReport ? (
                            ...
                            <span
                              ...
                              title="Pending - due Friday"
                            >
                              ·
                            </span>
                          ) : (
                            <span className="text-ink4 font-bold text-sm">
                              ✕
                            </span>
                          )}
                        </td>
                    ...
                    <td className="py-2.5 px-2 text-center">
                      {m.filesReport ? (
                        <span
                          className={`font-mono font-bold text-xs ${filedColor}`}
                        >
                          {filedCount}/{totalWeeks}
                        </span>
                      ) : (
                        <span className="text-ink4 text-xs">-</span>
                      )}
        ...
        <div className="card-body space-y-3">
          {BT_ITEMS.map((item) => (
            ...
            >
              ...
              <span
                className={`text-sm ${checks[item.key] ? "line-through text-ink4" : ""}`}
              >
                {item.label}
              </span>
            </div>
      ...
      <div className="card">
        ...
        <div className="card-body">
          {missing.length === 0 ? (
            !deadlinePassed && pending.length > 0 ? (
              <div className="text-sm text-ink3">
                {pending.length} report{pending.length > 1 ? "s" : ""} pending -
                due Friday. Not overdue yet.
              </div>
            ) : (
              <div className="text-sm text-ink3">
                All team members have filed ✓
              </div>
            )
          ) : (
            missing.map((m) => (
              <div
                key={m.name}
                className="flex items-center justify-between py-2.5 border-b border-sand3 last:border-0"
              >
                <div>
          ...
          <div className="mt-3 text-xs text-ink3">
            ...
            <a
              href="https://app.clickup.com/10643959/docs/a4ufq-50671"
              target="_blank"
              className="underline"
            >
              ClickUp Doc ↗
