...
function DownloadMenu({ text }: { text: string }) {
  ...
  const go = (fmt: ExportFormat) => {
    exportReport(text, fmt, deriveTitle(text));
    setOpen(false);
  };
  return (
    <div className="relative inline-block">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 text-xs font-semibold text-ink3 hover:text-accent transition-colors mt-2"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 mt-1 z-20 bg-sand border border-sand4 rounded-lg shadow-card-md py-1 w-36">
            <button
              onClick={() => go("pdf")}
              className="flex items-center gap-2 w-full px-3 py-2 text-sm hover:bg-sand3 text-left"
            >
              <FiFileText size={14} className="text-danger" /> PDF
            </button>
            <button
              onClick={() => go("doc")}
              className="flex items-center gap-2 w-full px-3 py-2 text-sm hover:bg-sand3 text-left"
            >
              <FiFile size={14} className="text-accent" /> Word (.doc)
            </button>
          </div>
        </>
      )}
...
function TypingDots() {
  return (
    <span className="inline-flex gap-1 py-1">
      <span
        className="w-1.5 h-1.5 bg-ink4 rounded-full animate-bounce"
...
function MessageRow({
  ...
  messages,
  userName,
}: RowComponentProps<{ messages: Msg[]; userName: string }>) {
        ...
        {m.role === "user" ? (
          <div className="flex gap-3 justify-end">
            <div className="flex flex-col items-end max-w-[85%]">
              <span className="text-[11px] font-semibold text-ink4 mb-1 mr-1">
                {userName}
              </span>
              <div className="bg-accent text-white rounded-2xl rounded-tr-sm px-5 py-3 text-sm whitespace-pre-wrap">
                {m.content}
              </div>
            </div>
            <div
              className="w-8 h-8 rounded-full bg-accent-light text-accent flex items-center justify-center flex-shrink-0 text-xs font-bold"
              title={userName}
            >
              {initialsOf(userName)}
            </div>
          ...
          <div className="flex gap-3">
            <div className="w-8 h-8 rounded-full bg-ink text-white flex items-center justify-center flex-shrink-0 font-display text-xs">
              AI
            </div>
...
function Modal({
  ...
  return (
    ...
    >
      <div
        className="bg-sand rounded-lg shadow-card-md border border-sand4 w-full max-w-lg max-h-[80vh] flex flex-col"
  ...
  return (
    <Modal title="Goals - Tony's source of truth" onClose={onClose}>
      <p className="text-xs text-ink4 mb-3">
        Quarterly objectives, ranked. Tony filters every recommendation against
        these and flags when work drifts.
      </p>
      {loading ? (
        ...
        <>
          <textarea
            ...
            rows={12}
            className="field-input resize-none font-mono text-[13px]"
            placeholder={
              "Q3 2026 objectives (most important first):\n1. Close FreshCredit - 17.5% commission\n2. Abakus $75M raise to first close\n3. ..."
            }
          />
...
function CommitmentsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Modal title="Decision & Commitment Log" onClose={onClose}>
      <p className="text-xs text-ink4 mb-3">
        Tony logs commitments and decisions from your chats automatically. Check
        one off when it&apos;s handled.
      </p>
      {loading ? (
        <div className="text-sm text-ink4">Loading…</div>
      ) : items.length === 0 ? (
        <div className="text-sm text-ink4">
          Nothing logged yet. Make a commitment in chat (e.g. &ldquo;I&apos;ll
          send the deck by Friday&rdquo;) and Tony will track it.
        </div>
      ) : (
                ...
                {open.map((c) => {
                  ...
                  return (
                    <button
                      key={c.id}
                      onClick={() => toggle(c)}
                      className="flex items-start gap-2.5 w-full text-left p-2 rounded hover:bg-sand2"
                    >
                      <FiSquare
                        size={16}
                        className="text-ink4 mt-0.5 flex-shrink-0"
                      />
              ...
              <div className="space-y-1.5">
                {done.map((c) => (
                  <button
                    key={c.id}
                    onClick={() => toggle(c)}
                    className="flex items-start gap-2.5 w-full text-left p-2 rounded hover:bg-sand2 opacity-60"
                  >
                    <FiCheckSquare
                      size={16}
  ...
  useEffect(() => {
    fetch("/api/chat/history")
      ...
      .then((d) => {
        ...
        setMessages(
          stored.map((m: { role: "user" | "assistant"; content: string }) => ({
            id: newId(),
            role: m.role,
            content: m.content,
          })),
  ...
  async function send(text: string) {
    ...
    try {
      ...
      for (;;) {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === asstId ? { ...m, content: stripLog(acc) } : m,
          ),
        );
      }
      ...
      if (wantFormat && clean.trim()) {
        try {
          exportReport(clean, wantFormat, deriveTitle(clean));
        } catch {
      ...
      setMessages((prev) =>
        prev.map((m) => (m.id === asstId ? { ...m, content: `⚠️ ${msg}` } : m)),
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="flex flex-col" style={{ height: "calc(100vh - 9rem)" }}>
      <div className="flex items-center justify-between mt-6 mb-3 gap-3">
        <div className="min-w-0">
          ...
          <p className="text-xs text-ink4 mt-0.5">
            Your Virtual Chief of Staff - remembers your chats, grounded in live
            data{isAdmin ? "" : " (your own data)"}. Create &amp; download
            reports as PDF or Word
            {isAdmin ? "" : "; team-wide reports are admin-only"}.
          </p>
        </div>
      ...
      <div className="flex-1 card mb-3 overflow-hidden flex flex-col min-h-0">
        {messages.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center py-8 px-4">
            <div className="w-12 h-12 rounded-full bg-ink text-white flex items-center justify-center font-display text-lg mb-3">
              AI
            </div>
            <p className="text-sm text-ink3 max-w-md mb-5">
              {firstName !== "You" ? `Hi ${firstName} - ` : ""}I&apos;m VCoS-AI,
              your Virtual Chief of Staff. I triage, draft, track, and flag -
              grounded in the current week&apos;s reports and ClickUp tasks, and
              I remember our past conversations. Ask me anything, find info, or
              generate a report. Tell me a format (“as PDF” / “as Word”) and
              I’ll prep a download.
            </p>
            <div className="flex flex-wrap gap-2 justify-center max-w-lg">
              {SUGGESTIONS.filter((s) => isAdmin || !s.admin).map((s) => (
                <button
                  ...
                  className="badge-accent hover:bg-accent hover:text-white transition-colors px-3 py-1.5"
                >
                  {s.label}
                </button>
              ))}
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 px-0.5 scrollbar-thin">
        <span className="text-[11px] font-semibold text-ink4 self-center flex-shrink-0 pr-0.5">
          Try:
        </span>
        {SUGGESTIONS.filter((s) => isAdmin || !s.admin).map((s) => (
          <button
      ...
      <div className="flex items-end gap-2">
        ...
        <button
          ...
          className="btn-primary flex items-center gap-2 h-[46px] disabled:opacity-40"
        >
